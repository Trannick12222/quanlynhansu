...
@app.route("/debug-epa-period")
def debug_epa_period():
    user = session.get('user')
    if not user:
        return f"No user in session"
    
    from apis.giaovien_epa import check_epa_period_for_user
    try:
        result = check_epa_period_for_user(user)
        return f"<pre>User: {user}\nResult: {result}</pre>"
    except Exception as e:
        import traceback
...
def delete_question(id):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT COUNT(*) as count FROM cauhoi_epa")
            result = cur.fetchone()
            count = result['count'] if isinstance(result, dict) else result[0]
            
            # If table is empty, reset AUTO_INCREMENT to 1
            if count == 0:
                cur.execute("ALTER TABLE cauhoi_epa AUTO_INCREMENT = 1")
                app.logger.info("Table cauhoi_epa is empty, reset AUTO_INCREMENT to 1")
...
@app.route('/api/cauhoi_epa/reset-auto-increment', methods=['POST'])
def reset_auto_increment():
    if "user" not in session:
        return jsonify({"error": "Chưa đăng nhập"}), 401
    
    # Chi cho phep admin hoac kimnhung
    if session["user"] not in ["admin", "kimnhung"]:
        return jsonify({"error": "Không có quyền thực hiện chức năng này"}), 403
    
    conn = get_conn()
    try:
        with conn.cursor() as cur:
            # Get current max ID
            cur.execute("SELECT MAX(id) as max_id FROM cauhoi_epa")
            result = cur.fetchone()
            max_id = result['max_id'] if isinstance(result, dict) and result['max_id'] else result[0] if result and result[0] else 0
            
            # Reset AUTO_INCREMENT to max_id + 1 (or 1 if table is empty)
            next_id = (max_id + 1) if max_id else 1
            cur.execute(f"ALTER TABLE cauhoi_epa AUTO_INCREMENT = {next_id}")
            
        conn.commit()
        app.logger.info(f"Reset AUTO_INCREMENT for cauhoi_epa to {next_id}")
        return jsonify({"success": True, "message": f"Đã reset AUTO_INCREMENT thành {next_id}", "next_id": next_id})
    except Exception as e:
        conn.rollback()
        app.logger.error(f"Error resetting AUTO_INCREMENT: {str(e)}")
        return jsonify({"success": False, "error": str(e)}), 500
    finally:
        conn.close()
...
def export_excel():
    """Export data to beautifully formatted Excel file"""
    try:
        import io
        from datetime import datetime
    ...
    # Get parameters
    dept = request.args.get('dept', '')
    if not dept or dept not in ['HS', 'GV']:
        return jsonify({'error': 'Invalid department parameter'}), 400
    ...
    try:
        ...
        if dept == 'HS':
            cursor.execute("""
                SELECT ho_va_ten, ngay_sinh, gioi_tinh, dan_toc, ma_dinh_danh,
            ...
            """)
            data = cursor.fetchall()
            headers = ['STT', 'Họ và Tên', 'Ngày Sinh', 'Giới Tính', 'Dân Tộc', 
                      'Mã Định Danh', 'Họ Tên Cha', 'Nghề Nghiệp Cha', 'Họ Tên Mẹ', 
                      'Nghề Nghiệp Mẹ', 'Địa Chỉ', 'CMND Cha Mẹ', 'SĐT Phụ Huynh']
            ...
            """)
            data = cursor.fetchall()
            headers = ['STT', 'Mã GV', 'Họ và Tên', 'Tên Gọi', 'Chức Vụ', 'Ngày Sinh',
                      'Quê Quán', 'CCCD', 'Ngày Cấp CCCD', 'Mã Số Thuế', 'Số CMND',
                      'Số BHXH', 'SĐT', 'Số TK', 'Email', 'Nhóm Máu', 'Địa Chỉ']
        ...
        ws['A1'] = 'UBND XÃ ĐINH VĂN - LÂM HÀ'
        ws['A1'].font = Font(size=10, bold=False)
        ws['A1'].alignment = Alignment(horizontal='left')
        
        ws.merge_cells('A2:C2')
        ws['A2'] = 'TRƯỜNG MẦM NON HOA HƯỚNG DƯƠNG'
        ws['A2'].font = Font(size=10, bold=True)
        ws['A2'].alignment = Alignment(horizontal='left')
        
        # Title
        title_range = f'A4:{get_column_letter(len(headers))}4'
        ...
        # Date
        date_str = f'Ngày xuất: {datetime.now().strftime("%d/%m/%Y %H:%M")}'
        date_range = f'A5:{get_column_letter(len(headers))}5'
        ...
        for row_idx, record in enumerate(data, start=header_row + 1):
            ...
            if dept == 'HS':
                values = [
                    record.get('ho_va_ten', ''),
                    record.get('ngay_sinh', '').strftime('%d/%m/%Y') if record.get('ngay_sinh') else '',
                    record.get('gioi_tinh', ''),
                    record.get('dan_toc', ''),
                    record.get('ma_dinh_danh', ''),
                    record.get('ho_ten_bo', ''),
                    record.get('nghe_nghiep_bo', ''),
                    record.get('ho_ten_me', ''),
                    record.get('nghe_nghiep_me', ''),
                    record.get('ho_khau', ''),
                    record.get('cccd_bo_me', ''),
                    record.get('sdt', '')
                ]
            else:  # GV
                values = [
                    ...
                    record.get('que_quan', ''),
                    record.get('cccd', ''),
                    record.get('ngay_cap', '').strftime('%d/%m/%Y') if record.get('ngay_cap') else '',
                    record.get('mst', ''),
                    record.get('cmnd', ''),
                    record.get('so_bh', ''),
                    record.get('sdt', ''),
        ...
        # Footer
        footer_row = len(data) + header_row + 2
        signature_range = f'{get_column_letter(len(headers)-2)}{footer_row}:{get_column_letter(len(headers))}{footer_row}'
        ws.merge_cells(signature_range)
        ws[f'{get_column_letter(len(headers)-2)}{footer_row}'] = f'Đinh Văn, ngày {datetime.now().day} tháng {datetime.now().month} năm {datetime.now().year}'
        ws[f'{get_column_letter(len(headers)-2)}{footer_row}'].alignment = Alignment(horizontal='center')
        
        signature_row = footer_row + 1
        ws.merge_cells(f'{get_column_letter(len(headers)-2)}{signature_row}:{get_column_letter(len(headers))}{signature_row}')
        ws[f'{get_column_letter(len(headers)-2)}{signature_row}'] = 'Người xuất dữ liệu'
        ws[f'{get_column_letter(len(headers)-2)}{signature_row}'].font = Font(bold=True)
        ws[f'{get_column_letter(len(headers)-2)}{signature_row}'].alignment = Alignment(horizontal='center')
...
def sup_epa_detail():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
                SELECT b.id, b.question, b.user_comment, b.user_score, b.sup_comment, b.sup_score,
                       COALESCE(c.score, 20) as max_score
                FROM bangdanhgia b
                LEFT JOIN cauhoi_epa c ON b.question = c.translate OR b.question = c.question
                WHERE b.ten_tk=%s AND b.year=%s AND b.month=%s
